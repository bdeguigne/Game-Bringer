import { getPopularGame, getScreenshots, storeGenresService } from "../services/homePageRequestsServices"

const { homePageRequestsConstants } = require("../constants/homePageRequestsConstants");

const checkPopularGamesComplete = () => {
    return (dispatch, getState) => {
        var popularGames = getState().homePageRequest.popularGamesData;
        var genres = getState().homePageRequest.genres;
        ...
        if (popularGames.length === 10 && genres.length > 0) {
            ...
            dispatch({
                type: homePageRequestsConstants.POPULAR_GAMES_COMPLETE
            })
        }
    }
}
    ...
    return dispatch => {
        ...
        getScreenshots(screenShotID)
            ...
            .then(result => {
                ...
                dispatch({
                    type: homePageRequestsConstants.ADD_POPULAR_GAME_DATA,
                    data
                })
...
//                                 dispatch({
//                                     type: homePageRequestsConstants.ADD_POPULAR_GAME_COMPANY,
//                                     company: data
//                                 })
//                             })
...
export const storeGenres = () => {
    return dispatch => {
        storeGenresService()
            ...
            .then(result => {
                dispatch({
                    type: homePageRequestsConstants.STORE_GENRES,
                    genres: result
                })
                dispatch(checkPopularGamesComplete());
            })
...
export function getPopularGames() {
    return (dispatch, getState) => {
        var isRequestComplete = getState().homePageRequest.isRequestComplete;
